...
class File {
       public:
        ...
        // Keep this for the Root Merger reading from the actual file
        ...
        // NEW FIX: Use raw Unix system calls to bypass poisoned std::cin
        void read_all_from_fd(int fd) {
                char buffer[4096];
                ssize_t bytes_read;
        ...
        void write_interval(int start_line, int end_line, int file_index) {
                ...
                if (num_lines == 0) {
                        fprintf(stderr, "EXITTED: Asked lines from %d to %d but file has 0 lines\n", start_line, end_line);
                        exit(1);
                }
                ...
                for (int i = start_line - 1; i < end_line; i++) {
                        ...
                        ssize_t to_write = line.length();
                        ssize_t written = 0;

                        // FIX: Loop until every single byte is safely pushed into the pipe
                        while (written < to_write) {
                                ssize_t w = write(file_index, line.c_str() + written, to_write - written);
                                if (w <= 0) break;
                                written += w;
                        }
                }
...
void read_pipe_until_EOF(int read_fd, std::vector<std::string>& exit_status_lines) {
        char buffer[4096];
        ssize_t bytes_read;
        std::string current_line;
        ...
        while ((bytes_read = read(read_fd, buffer, sizeof(buffer))) > 0) {
                for (ssize_t i = 0; i < bytes_read; i++) {
                        current_line += buffer[i];
                        if (buffer[i] == '\n') {
                                if (current_line.substr(0, 11) == "EXIT-STATUS") {
                                        exit_status_lines.push_back(current_line);
                                } else {
                                        write(STDOUT_FILENO, current_line.c_str(), current_line.size());
                                }
                                current_line.clear();
                        }
                }
...
void sub_merger(merger_node_t* root, File file) {
        ...
        for (int chain_num = 0; chain_num < num_chains; chain_num++) {
                ...
                if (is_submerge) {
                        ...
                        if (PIPE(P_to_sub) < 0) {
                                perror("Fatal: P_to_sub failed");
                                exit(1);
                        }
                        if (PIPE(P_from_sub) < 0) {
                                perror("Fatal: P_from_sub failed");
                                exit(1);
                        }
                        ...
                        for (int j = 0; j < num_pipes; j++) {
                                if (PIPE(file_dscrs[j].data()) < 0) {
                                        perror("Fatal: file_dscrs PIPE failed");
                                        exit(1);
                                }
                        }
                        ...
                        for (int op_num = 0; op_num < num_ops; op_num++) {
                                if ((chain_child_pids[chain_num][op_num] = fork()) == 0) {
                                        ...
                                        for (int i = 0; i < file_dscrs.size(); i++) {
                                                // fprintf(stderr, "%d th file descr are %d, %d", i, file_dscrs[i][0], file_dscrs[i][0]);
                                        }
        ...
        for (int chain_num = 0; chain_num < num_chains; chain_num++) {
                ...
                if (f_pid == 0) {
                        ...
                        // FIX: Sub-mergers must receive the entire data block!
                        file.write_interval(cur_chain.start_line, cur_chain.end_line, input_write_fds[chain_num]);

                        // Safely close the write-end so the reader receives EOF
        ...
        // 3.3: Read chain outputs
        std::vector<std::vector<std::string>> per_chain_exit_statuses(num_chains);
        for (int chain_num = 0; chain_num < num_chains; chain_num++) {
                read_pipe_until_EOF(output_read_fds[chain_num], per_chain_exit_statuses[chain_num]);
        ...
        // 4.3: Reap and print EXIT-STATUS in strict chain order
        // write Exit status at the end so that reserve the order of print
        // first data then exit status
        // extract exit status from each chains output in order then
        // write in the same order
        for (int chain = 0; chain < num_chains; chain++) {
                // First print this chain's own operator exit statuses
                ...
                // Then print bubbled exit statuses from sub-merger for this chain
                for (const std::string& line : per_chain_exit_statuses[chain]) {
